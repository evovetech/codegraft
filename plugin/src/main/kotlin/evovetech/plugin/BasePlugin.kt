...
class BasePlugin<T : Any>(
    ...
    var app by once<AppExtension<T>> {
        throw IllegalStateException("app not set")
    }

    final override
    fun apply(target: T) = target.run {
        println("${this@BasePlugin::class.simpleName} { target=$this }")
        app = extensions!!.create("app", extType.java, this)
        initialize()
    }
...
val Settings.app: SettingsAppExtension
    get() {
        val app = extensions?.get("app")
        return when (app) {
            is SettingsAppExtension -> app
            else -> throw IllegalStateException("app not set for $this")
        }
    }

val Project.app: ProjectAppExtension
    get() {
        val app = extensions["app"]
        return when (app) {
            is ProjectAppExtension -> app
            else -> throw IllegalStateException("app not set for $this")
        }
    }
