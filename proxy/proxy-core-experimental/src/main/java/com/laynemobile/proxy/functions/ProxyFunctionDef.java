...
public class ProxyFunctionDef<P, F extends ProxyFunctionTransform<P, ?, R>, R> {
    ...
    public ProxyFunctionDef(String name, TypeToken<R> returnType, TypeToken<?>[] paramTypes) {
    ...
    public ProxyFunction2<P, F, R> asFunction(F transform) {
        return new ProxyFunction2<>(this, transform);
    }

    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof ProxyFunctionDef)) return false;
        ProxyFunctionDef<?, ?, ?> that = (ProxyFunctionDef<?, ?, ?>) o;
