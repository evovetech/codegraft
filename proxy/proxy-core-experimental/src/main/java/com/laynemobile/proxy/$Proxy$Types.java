...
final class $Proxy$Types {
    private static final String TAG = $Proxy$Types.class.getSimpleName();
    ...
    /**
     * Returns a new parameterized type, applying {@code typeArguments} to {@code rawType} and enclosed by {@code
     * ownerType}.
     *
     ...
     * {@code CharSequence.class}, this returns {@code ? extends CharSequence}. If {@code bound} is {@code
     * Object.class}, this returns {@code ?}, which is shorthand for {@code ? extends Object}.
     */
    ...
    /**
     * Returns a type that represents an unknown supertype of {@code bound}. For example, if {@code bound} is {@code
     * String.class}, this returns {@code ? super String}.
     */
    ...
    static Class<?> getRawType(Type type) {
        if (type instanceof Class<?>) {
            ...
            Util.checkArgument(rawType instanceof Class);
            ...
            if (bounds.length == 1) {
                ProxyLog.d(TAG, "getting typevariable bound raw type: %s", bounds[0]);
     ...
     * for when supertype is {@code Set.class} is {@code Set<Integer>} and the result when the supertype is {@code
     * Collection.class} is {@code Collection<Integer>}.
     */
    ...
    private static void checkNotPrimitive(Type type) {
        Util.checkArgument(!(type instanceof Class<?>) || !((Class<?>) type).isPrimitive());
    }
    ...
    private static final class ParameterizedTypeImpl implements ParameterizedType, Serializable {
        ...
        ParameterizedTypeImpl(Type ownerType, Type rawType, Type... typeArguments) {
            ...
            if (rawType instanceof Class<?>) {
                ...
                Util.checkArgument(ownerType != null || isStaticOrTopLevelClass);
            }
            ...
            for (int t = 0; t < this.typeArguments.length; t++) {
                Util.checkNotNull(this.typeArguments[t]);
    ...
    private static final class WildcardTypeImpl implements WildcardType, Serializable {
        ...
        public WildcardTypeImpl(Type[] upperBounds, Type[] lowerBounds) {
            Util.checkArgument(lowerBounds.length <= 1);
            Util.checkArgument(upperBounds.length == 1);

            if (lowerBounds.length == 1) {
                Util.checkNotNull(lowerBounds[0]);
                checkNotPrimitive(lowerBounds[0]);
                Util.checkArgument(upperBounds[0] == Object.class);
                this.lowerBound = canonicalize(lowerBounds[0]);
                this.upperBound = Object.class;
            } else {
                Util.checkNotNull(upperBounds[0]);
