...
class AliasElements {
    ...
    public static
    ElementAlias get(Element element) {
        return getInternal(element);
    }

    public static
    NameAlias get(Name name) {
        return DefaultNameAlias.of(name);
    }

    public static
    PackageElementAlias get(PackageElement element) {
        return getInternal(element);
    }

    public static
    TypeElementAlias get(TypeElement element) {
        return getInternal(element);
    }

    public static
    ExecutableElementAlias get(ExecutableElement element) {
        return getInternal(element);
    }

    public static
    TypeParameterElementAlias get(TypeParameterElement element) {
        return getInternal(element);
    }

    public static
    VariableElementAlias get(VariableElement element) {
        return getInternal(element);
    }

    public static
    ImmutableList<? extends ElementAlias> elements(List<? extends Element> elements) {
        return buildList(elements);
    }

    public static
    AnnotationMirrorAlias get(AnnotationMirror annotationMirror) {
        return DefaultAnnotationMirrorAlias.of(annotationMirror);
    }

    public static
    ImmutableList<? extends AnnotationMirrorAlias> annotationMirrors(
    ...
    public static
    ImmutableList<? extends TypeParameterElementAlias> typeParameters(
    ...
    public static
    ImmutableList<? extends VariableElementAlias> parameters(List<? extends VariableElement> parameters) {
        return buildList(parameters);
    }

    @SuppressWarnings("unchecked")
    private static
    <K extends Element, V extends ElementAlias> V getInternal(K k) {
    ...
    private static
    <K extends Element, V extends ElementAlias> ImmutableList<? extends V> buildList(
            List<? extends K> in
    ) {
    ...
    class Cache
            extends AbstractCache<Element, ElementAlias>
    {
        private
        Cache() {}

        @Override protected
        ForwardingAlias createFutureValue(Element element) {
            return new ForwardingAlias();
        }

        @Override protected
        ElementAlias create(Element element) {
            log("visiting %s - %s", element.getKind(), element.getSimpleName());
            return element.accept(new Visitor8(), null);
        }
    ...
    private static final
    class Visitor8
            extends SimpleElementVisitor8<ElementAlias, Void>
    {
        private
        Visitor8() {}

        private
        Visitor8(ElementAlias defaultValue) {
            super(defaultValue);
        }

        @Override protected
        ElementAlias defaultAction(Element e, Void aVoid) {
        ...
        @Override public
        ElementAlias visitType(TypeElement e, Void aVoid) {
            return DefaultTypeElementAlias.of(e);
        }

        @Override public
        ElementAlias visitTypeParameter(TypeParameterElement e, Void aVoid) {
            return DefaultTypeParameterElementAlias.of(e);
        }

        @Override public
        ElementAlias visitVariable(VariableElement e, Void aVoid) {
            return DefaultVariableElementAlias.of(e);
        }

        @Override public
        ElementAlias visitExecutable(ExecutableElement e, Void aVoid) {
            return DefaultExecutableElementAlias.of(e);
        }

        @Override public
        ElementAlias visitPackage(PackageElement e, Void aVoid) {
            return DefaultPackageElementAlias.of(e);
        }
    }

    private static final
    class ForwardingAlias
            ...
            AbstractCache.FutureValue<TypedElementAlias<?>>
    {
        ...
        @Override public
        void setDelegate(TypedElementAlias<?> delegate) {
        ...
        @Override public
        List<? extends TypeMirrorAlias> getInterfaces() {
        ...
        @Override public
        NameAlias getQualifiedName() {
            return typeElement().getQualifiedName();
        }

        @Override public
        TypeMirrorAlias getSuperclass() {
            return typeElement().getSuperclass();
        }

        @Override public
        List<? extends TypeParameterElementAlias> getTypeParameters() {
        ...
        @Override public
        List<? extends TypeMirrorAlias> getBounds() {
            return typeParameterElement().getBounds();
        }

        @Override public
        ElementAlias getGenericElement() {
        ...
        @Override public
        AnnotationValueAlias getDefaultValue() {
            return executableElement().getDefaultValue();
        }

        @Override public
        List<? extends VariableElementAlias> getParameters() {
        ...
        @SuppressWarnings("unchecked")
        private
        <T extends ElementAlias> T cast(String message) {
            try {
                return (T) ensure();
        ...
        private
        PackageElementAlias packageElement() {
            return cast("not a PackageElementAlias");
        }

        private
        TypeElementAlias typeElement() {
            return cast("not a TypeElementAlias");
        }

        private
        TypeParameterElementAlias typeParameterElement() {
            return cast("not a TypeParameterElementAlias");
        }

        private
        ExecutableElementAlias executableElement() {
            return cast("not an ExecutableElementAlias");
        }

        private
        VariableElementAlias variableElement() {
