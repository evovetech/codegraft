        ...
        implements Elements
{
    ...
    public static
    EnvElements with(Env env) {
        return new EnvElements(env);
    }

    @Override public
    ImmutableList<? extends AnnotationMirrorAlias> getAllAnnotationMirrors(Element e) {
        return AliasElements.annotationMirrors(elementUtils.getAllAnnotationMirrors(e));
    }

    @Override public
    ImmutableList<? extends ElementAlias> getAllMembers(TypeElement type) {
        return AliasElements.elements(elementUtils.getAllMembers(type));
    }

    @Override public
    NameAlias getBinaryName(TypeElement type) {
    ...
    @Override
    public
    ImmutableMap<? extends ExecutableElementAlias, ? extends AnnotationValueAlias> getElementValuesWithDefaults(
            AnnotationMirror a
    ) {
    ...
    @Override public
    NameAlias getName(CharSequence cs) {
        return AliasElements.get(elementUtils.getName(cs));
    }

    @Override public
    PackageElementAlias getPackageElement(CharSequence name) {
        return AliasElements.get(elementUtils.getPackageElement(name));
    }

    @Override public
    PackageElementAlias getPackageOf(Element type) {
        return AliasElements.get(elementUtils.getPackageOf(type));
    }

    @Override public
    TypeElementAlias getTypeElement(CharSequence name) {
