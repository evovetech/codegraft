...
class LibComponentEnv(env: LibModuleEnv) : BaseEnv(env) {
    ...
    override fun readAnnotations(annotatedElements: Set<Element>) {
        for (annotatedElement in annotatedElements) {
            ...
            te.enclosedElements.annotatedWith<LibComponent.Builder, Element>()
                    .mapNotNull(AnnotatedElement<*, *>::asTypeElement)
                    .map(AnnotatedTypeElement<*>::element)
                    .mapTo(classes.builders, ClassName::get)
        }
    }
    ...
    object Parser : Reader.Parser<LibComponentElement> {
        override fun parse(reader: Reader): LibComponentElement {
            ...
            val dependencies = reader.readTypeNames()
                    .map { it as ClassName }
            val builders = reader.readTypeNames()
                    .map { it as ClassName }
            return LibComponentElement(className, modules, dependencies, builders)
        }
    }
}
