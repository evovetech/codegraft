...
class Mirrors {
    ...
    Name getQualifiedName(DeclaredType type) {
        return type.asElement().accept(new SimpleElementVisitor6<Name, Void>() {
            @Override
            protected
            Name defaultAction(Element e, Void p) {
                throw new AssertionError("DeclaredTypes should be TypeElements");
            }

            @Override
            public
            Name visitType(TypeElement e, Void p) {
     ...
     */
    static
    ImmutableMap<String, AnnotationValue> simplifyAnnotationValueMap(
    ...
    static
    <T> Optional<Equivalence.Wrapper<T>> wrapOptionalInEquivalence(
     ...
     */
    static
    <T> Optional<T> unwrapOptionalEquivalence(
